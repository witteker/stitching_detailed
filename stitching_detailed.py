...
def main():
    ...
    for name in img_names:
        ...
        if work_megapix < 0:
            ...
            if is_work_scale_set is False:
                ...
                is_work_scale_set = True
            img = cv.resize(src=full_img, dsize=None, fx=work_scale,
                            fy=work_scale, interpolation=cv.INTER_LINEAR_EXACT)
        if is_seam_scale_set is False:
            seam_scale = min(1.0, np.sqrt(seam_megapix * 1e6 /
                             (full_img.shape[0] * full_img.shape[1])))
            seam_work_aspect = seam_scale / work_scale
            is_seam_scale_set = True
        img_feat = cv.detail.computeImageFeatures2(finder, img)
        features.append(img_feat)
        img = cv.resize(src=full_img, dsize=None, fx=seam_scale,
    ...
    for idx, name in enumerate(img_names):
        ...
        seam_mask = cv.resize(
            dilated_mask, (mask_warped.shape[1], mask_warped.shape[0]), 0, 0, cv.INTER_LINEAR_EXACT)
        ...
        if timelapse:
            ...
            if pos_s == -1:
                ...
                fixed_file_name = img_names[idx][:pos_s +
                                                 1] + "fixed_" + img_names[idx][pos_s + 1:]
    ...
    if not timelapse:
        ...
        dst = cv.normalize(src=result, dst=None, alpha=255.,
